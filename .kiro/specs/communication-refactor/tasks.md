...
- [x] 1. 扩展类型定义和 AgentConfig
  - [x] 1.1 扩展 `xar/src/agent/types.ts` 中的 AgentConfig
  ...
  - [x] 1.2 扩展 `xar/src/types.ts` 中的 InboundMessage
    - 新增 `mentioned?: boolean` 和 `conversation_type?: string` 可选字段
    - _Requirements: 9.3_

  - [x] 1.3 新增 Task/SubTask 类型定义到 `xar/src/agent/task-types.ts`
    - 定义 Task、SubTask、CreateTaskParams、AnnounceResult 接口
    - _Requirements: 1.1_

- [x] 2. 实现 Task Manager 核心模块
  - [x] 2.1 实现 `xar/src/agent/task-manager.ts`
  ...
  - [x]* 2.2 编写 Task Manager 属性测试 `xar/vitest/pbt/task-manager.pbt.test.ts`
  ...
  - [x]* 2.3 编写 Task Manager 单元测试 `xar/vitest/unit/task-manager.test.ts`
    - 测试边界情况：空 subtask 列表、单 subtask、重复 announce、task 不存在、task 已完成时取消
    - _Requirements: 1.1, 2.1, 2.3_

- [x] 3. 重构 Router 模块
  - [x] 3.1 重构 `xar/src/agent/router.ts` 的 determineThreadId 函数
  ...
  - [x] 3.2 新增 determineEventType 函数到 `xar/src/agent/router.ts`
    - 根据 AgentConfig 的 mode + trigger 和消息的 conversation_type + mentioned 决定 event_type
    - _Requirements: 3.3, 3.4, 3.5, 9.2_

  - [x]* 3.3 编写 Router 属性测试 `xar/vitest/pbt/router.pbt.test.ts`
  ...
  - [x]* 3.4 编写 Router 单元测试 `xar/vitest/unit/router.test.ts`
...
- [x] 5. 重构 Communication Context 模块
  - [x] 5.1 实现角色检测函数 detectRole 到 `xar/src/agent/context.ts`
    - 根据 source kind、reply_to、taskContext 动态判定角色
    - 返回 AgentRole 类型
    - _Requirements: 5.1, 5.2, 5.3, 5.4, 5.5_

  - [x] 5.2 重构 buildCommunicationContext 函数
  ...
  - [x]* 5.3 编写 Context 属性测试 `xar/vitest/pbt/context.pbt.test.ts`
    - **Property 7: 角色检测正确性**
    - **Validates: Requirements 5.1, 5.2, 5.3, 5.4**
    - **Property 8: Communication Context 生成正确性**
    - **Validates: Requirements 6.1, 6.2, 6.3, 6.4, 6.5, 6.6, 6.7**

  - [x]* 5.4 编写 Context 单元测试 `xar/vitest/unit/context.test.ts`
    - 测试各场景模板的具体输出内容
    - _Requirements: 6.1, 6.2, 6.3, 6.4, 6.5, 6.6, 6.7_

- [x] 6. 实现 create_task 和 cancel_task 工具
  - [x] 6.1 实现 `xar/src/agent/create-task.ts`
...
- [x] 8. 实现 Mid-Turn Injection 模块
  ...
  - [x]* 8.2 编写 Mid-Turn 属性测试 `xar/vitest/pbt/mid-turn.pbt.test.ts`
    - **Property 10: Mid-Turn Injection 构造正确性**
    - **Validates: Requirements 7.2**

  - [x]* 8.3 编写 Mid-Turn 单元测试 `xar/vitest/unit/mid-turn.test.ts`
...
- [x] 10. 重构 Run-Loop 状态机
  - [x] 10.1 重构 `xar/src/agent/run-loop.ts` 的 processMessage 方法
  ...
  - [x] 10.2 更新 system prompt 模板
    - 添加 receive_user_update 机制说明
    - _Requirements: 7.4_

  - [x]* 10.3 编写 Run-Loop 集成测试 `xar/vitest/unit/run-loop.test.ts`
...
- [x] 11. xgw 侧变更
  - [x] 11.1 修改 `xgw/src/gateway/server.ts`
