...
- [x] 2. 定义共享类型与数据模型
  - [x] 2.1 创建 `src/types.ts`，定义所有共享类型
...
- [x] 4. 实现 `src/profile-loader.ts` Profile 加载器
  - [x] 4.1 实现 Profile 解析与占位符处理核心函数
  ...
  - [x] 4.4 创建内置 profile 文件
    - 创建 `profiles/minimal.yaml`
    - 创建 `profiles/standard.yaml`
    - _需求：3.7_

- [x] 5. 实现 `src/component-manager.ts` 组件管理器
  - [x] 5.1 实现 ComponentsYaml 解析与版本检测
  ...
  - [x] 5.2 为 ComponentsYaml 解析编写属性测试
...
- [x] 7. 实现 `src/commands/setup.ts` Setup 命令
  - [x] 7.1 实现 setup 步骤执行器与幂等性逻辑
...
- [x] 8. 实现 `src/commands/status.ts` Status 命令
  - [x] 8.1 实现状态聚合与格式化逻辑
...
- [x] 9. 实现 `src/commands/upgrade.ts` Upgrade 命令
  - [x] 9.1 实现版本比较与升级执行逻辑
...
- [x] 11. 实现 `src/index.ts` CLI 入口与命令注册
